...
class SpecificationController {
    ...
    async getData() {
        try {
            var data = await Specification.find({});
            return ({
                Message: "Data Geted",
                Data: data,
                Code: 200
    ...
    async getDataById(id) {
        try {
            var idData = await Specification.findById(id);
            return ({
    ...
    deleteData = async (id) => {
        try {
            const idData = await Specification.findByIdAndDelete(id);
            return ({
                Data: idData,
    ...
    updateData = async (id, body) => {
        try {
            const updateData = await Specification.findByIdAndUpdate(id, body);
            return ({
                Data: updateData,
